...
def generator(A,classes, means,cov):
    num_node = A.shape[0]
...
def generate_dataset(A, classes, size, means, cov):
    Attributes = []
    Labels = []
    for __ in range(size):
        attributes,labels,_ = generator(A,classes,means,cov)
...
if __name__ == "__main__":
    ...
    classes = cut_graph(A,5)
    class_0,class_1,class_2,class_3,class_4 = classes

    attributes, labels, classes = generator(A, classes,means =means, cov =cov)
    draw_colored_graph(A, class_0, class_1,class_2,class_3,class_4)

    Attributes, Labels = generate_dataset(A=A, classes = classes, size =100, means=means, cov=cov)
