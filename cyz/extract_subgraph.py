...
def get_subgraph(num_of_node_with_large_degree_per_class):
    '''
    Parameter:
        num_of_node_with_large_degree_per_class: Integer, the number of nodes that are selected based on their rank in the degree
    Return:
        reselected_G: selected subgraph, Graph
        final_nodes: node indexes in the ascending order, 1d-array
    ...
    selected_num = num_of_node_with_large_degree_per_class
    num_of_class = 7
    ...
    A = nx.adjacency_matrix(reselected_G, nodelist=final_nodes)
    A = A.todense()
    # check the symmetry of adjacency matrix
    tol = 1e-8
    isSymmetry = np.all(np.abs(A-A.T) < tol)
    print("Adjacency matrix A is symmetric: ", isSymmetry)
    return reselected_G, final_nodes, A

if __name__ == "__main__":
    reselected_G, final_nodes, A = get_subgraph(num_of_node_with_large_degree_per_class=30)